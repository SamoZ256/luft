    ...
    auto integer(enumeration value, bool is_signed = false) {
        factories.push_back([=](llvm::LLVMContext& context) {
            return ConstantAsMetadata::get(ConstantInt::get(
                context, APInt{32, (uint32_t)value, is_signed}));
        });
        return this;
    };
    auto metadata(llvm::Metadata* metadata) {
        factories.push_back(
            [=](llvm::LLVMContext& context) { return metadata; });
        return this;
    };
...
uint32_t ArgumentBufferBuilder::DefineBuffer(
    ...
    fieldsType.push_back(ArgumentBindingBuffer{
        .location_index = location_index == UINT32_MAX ? (uint32_t)element_index
                                                       : location_index,
        .array_size = 1,
    ...
    fieldsType.push_back(ArgumentBindingTexture{
        ...
        .type =
            MSLTexture{
                ...
                .memory_access = access,
                .resource_kind = lowering_texture_1d_to_2d(kind),
...
uint32_t ArgumentBufferBuilder::DefineInteger64(std::string name,
                                                uint32_t location_index) {
    auto element_index = fieldsType.size();
    assert(!fields.count(name) && "otherwise duplicated field name");
...
auto build_argument_binding_buffer(StreamMDHelper& md,
...
auto build_argument_binding_indirect_buffer(
    ...
    llvm::LLVMContext& context, const llvm::DataLayout& layout) -> llvm::Type* {
    ...
    return indirect_buffer.struct_type->getPointerTo(
        (uint32_t)indirect_buffer.address_space);
};
auto build_argument_binding_texture(StreamMDHelper& md,
    ...
    case luft::MemoryAccess::read:
        md.string("air.read");
        break;
    case luft::MemoryAccess::write:
        md.string("air.write");
        break;
    case luft::MemoryAccess::read_write:
        md.string("air.read_write");
        break;
    case luft::MemoryAccess::sample:
...
auto build_argument_binding_sampler(StreamMDHelper& md,
...
auto insert_inteterpolation(StreamMDHelper& md, Interpolation interpolation,
    ...
    if (pull_mode) {
        ...
        case Interpolation::sample_no_perspective:
            md.string("air.interpolation_function")
                ->string("air.no_perspective");
            break;
...
auto ArgumentBufferBuilder::Build(llvm::LLVMContext& context,
    ...
    for (auto argument : fieldsType) {
        ...
        auto field_type = std::visit(
            patterns{[&](const ArgumentBindingBuffer& buffer) {
    ...
    for (auto [field_type, member_offset, indirect_argument_metadata] :
        ...
        struct_metadata.integer(member_offset)
            ->integer(layout.getTypeSizeInBits(field_type) /
                      8) // CAVEATS: the bool is represented as i8 in arugment
                         // buffer (but i1 in function argument)
            ->integer(0) // TODO: array_size (0 for non-array, like
                         // MTLArguementDescriptor)
            ->string(cast<MDString>(cast<MDTuple>(indirect_argument_metadata)
                                        ->operands()
                                        .end()[-3])
                         ->getString()
...
uint32_t FunctionSignatureBuilder::DefineInput(const FunctionInput& input) {
    ...
    for (const auto& element : enumerate(inputs)) {
        if (element.value().index() == input.index()) {
            if (std::visit(
                    patterns{
                        [&](const InputVertexStageIn s) {
                            return s.attribute ==
                                   std::get<InputVertexStageIn>(element.value())
                                       .attribute;
                        },
                        [&](const InputFragmentStageIn s) {
                            return s.user == std::get<InputFragmentStageIn>(
                                                 element.value())
                                                 .user;
                        },
                        [&](const ArgumentBindingBuffer s) {
                            return s.location_index ==
                                   std::get<ArgumentBindingBuffer>(
                                       element.value())
                                       .location_index;
                        },
                        [&](const ArgumentBindingSampler s) {
                            return s.location_index ==
                                   std::get<ArgumentBindingSampler>(
                                       element.value())
                                       .location_index;
                        },
                        [&](const ArgumentBindingTexture s) {
                            return s.location_index ==
                                   std::get<ArgumentBindingTexture>(
                                       element.value())
                                       .location_index;
                        },
                        [&](const ArgumentBindingIndirectBuffer s) {
                            return s.location_index ==
                                   std::get<ArgumentBindingIndirectBuffer>(
                                       element.value())
                                       .location_index;
                        },
                        [](auto) { return true; }},
...
uint32_t FunctionSignatureBuilder::DefineMeshVertexOutput(
    const MeshVertexOutput& output) {
...
uint32_t FunctionSignatureBuilder::DefineMeshPrimitiveOutput(
    const MeshPrimitiveOutput& output) {
...
auto FunctionSignatureBuilder::CreateFunction(std::string name,
                                              llvm::LLVMContext& context,
                                              llvm::Module& module,
                                              uint64_t sign_mask,
                                              bool skip_output)
    ...
    auto get_input_attribute_type =
        [](InputAttributeComponentType type, uint32_t attribute,
           uint64_t sign_mask) -> MSLScalerOrVectorType {
        switch (type) {
    ...
    for (const auto& item : enumerate(mesh_vertex_outputs)) {
    ...
    for (const auto& item : enumerate(mesh_primitive_outputs)) {
        auto output = item.value();
        StreamMDHelper md;
        std::visit(patterns{
    ...
    for (const auto& item : enumerate(inputs)) {
        auto i = item.index();
        ...
        llvm::Type* field_type = std::visit(
            patterns{
                ...
                [&](const InputFragmentStageIn& frag_in) -> llvm::Type* {
                    ...
                    insert_inteterpolation(metadata_field,
                                           frag_in.interpolation,
                                           frag_in.pull_mode);
                    ...
                    if (frag_in.pull_mode)
                        return getOrCreateStructType("struct._interpolant_t",
                                                     context)
                            ->getPointerTo(1);
                ...
                [&](const ArgumentBindingBuffer& buffer) {
                    return build_argument_binding_buffer(
                        metadata_field, buffer, context,
                        module.getDataLayout());
                },
                [&](const ArgumentBindingIndirectBuffer& indirect_buffer) {
                    return build_argument_binding_indirect_buffer(
                        metadata_field, indirect_buffer, context,
                        module.getDataLayout());
                },
                [&](const ArgumentBindingTexture& texture) {
                    return build_argument_binding_texture(metadata_field,
                                                          texture, context);
                },
                [&](const ArgumentBindingSampler& sampler) {
                    return build_argument_binding_sampler(metadata_field,
                                                          sampler, context);
                },
                [&](const InputPosition& position) {
                    metadata_field.string("air.position");
                    insert_inteterpolation(metadata_field,
                                           position.interpolation, false);
                    metadata_field.string("air.arg_type_name")
                ...
                [&](const InputMeshGridProperties&) -> llvm::Type* {
                    ...
                    return getOrCreateStructType(
                               "struct._mesh_grid_properties_t",
                               context)
                        ->getPointerTo( //
                ...
                [&](const InputMesh& mesh) -> llvm::Type* {
                    ...
                    mesh_info.string("air.mesh_type_info")
                        ->metadata(
                            MDTuple::get(context, metadata_mesh_vertex_output))
                        ->metadata(MDTuple::get(context,
                                                metadata_mesh_primitive_output))
                    ...
                    metadata_field.string("air.mesh")
                        ...
                        ->string("air.arg_name")
                        ->string("mtl_mesh");
                    return getOrCreateStructType("struct._mesh_t", context)
                        ->getPointerTo(7);
                },
                [](auto _) {
    ...
    for (const auto& item : enumerate(outputs)) {
        if (skip_output)
            continue;
        auto output = item.value();
        StreamMDHelper md;
        llvm::Type* field_type = std::visit(
            patterns{[&](const OutputVertex& vertex_out) {
                     ...
                     [&](const OutputRenderTarget& render_target) {
                         if (render_target.dual_source_blending) {
                             md.string("air.render_target")
                                 ...
                                 ->string("air.arg_name")
                                 ->string("render_target_" +
                                          std::to_string(render_target.index));
                     ...
                     [&](const OutputDepth& depth) {
                         ...
                         md.string("air.depth")
                             ...
                             ->string(
                                 qualifiers[(uint32_t)depth.depth_argument])
                             ->string("air.arg_type_name")
                             ->string("float")
                             ->string("air.arg_name")
                             ->string("mtl_depth");
