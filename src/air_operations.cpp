...
std::string type_overload_suffix(llvm::Type* type,
    ...
    if (type->getTypeID() == llvm::Type::FixedVectorTyID) {
        ...
        if (elementTy->isFloatTy()) {
            ...
            assert(llvm::cast<llvm::IntegerType>(elementTy)->getBitWidth() ==
                   32);
            if (sign == Sign::inapplicable)
                return ".v" + std::to_string(fixedVecTy->getNumElements()) +
                       "i32";
            return sign == Sign::with_sign
                       ? ".s.v" + std::to_string(fixedVecTy->getNumElements()) +
                             "i32"
                       : ".u.v" + std::to_string(fixedVecTy->getNumElements()) +
                             "i32";
        }
    }
...
AIRBuilderResult call_float_unary_op(std::string op, pvalue a) {
    return make_op([=](AIRBuilderContext ctx) {
        ...
        auto operand_type = a->getType();
        auto fn = (module.getOrInsertFunction(
            "air." + fastmath_variant(ctx, op) +
                type_overload_suffix(operand_type),
            llvm::FunctionType::get(operand_type, {operand_type}, false), att));
        return ctx.builder.CreateCall(fn, {a});
    });
};

AIRBuilderResult call_integer_binop(std::string op, pvalue a, pvalue b,
                                    bool is_signed) {
    return make_op([=](AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
        ...
        auto fn = (module.getOrInsertFunction(
            "air." + op +
                type_overload_suffix(operand_type, is_signed ? Sign::with_sign
                                                             : Sign::no_sign),
            llvm::FunctionType::get(operand_type, {operand_type, operand_type},
                                    false),
            att));
...
AIRBuilderResult call_dot_product(uint32_t dimension, pvalue a, pvalue b) {
    return make_op([=](AIRBuilderContext ctx) {
        ...
        auto fn = (module.getOrInsertFunction(
            ...
            llvm::FunctionType::get(types._float, {operand_type, operand_type},
                                    false),
            att));
        return ctx.builder.CreateCall(fn, {a, b});
    });
...
AIRBuilderResult call_float_mad(pvalue a, pvalue b, pvalue c) {
    return make_op([=](AIRBuilderContext ctx) -> pvalue {
        ...
        auto fn = (module.getOrInsertFunction(
            ...
            llvm::FunctionType::get(operand_type,
                                    {operand_type, operand_type, operand_type},
                                    false),
            att));
    ...
    return make_op([=](AIRBuilderContext ctx) {
        ...
        auto fn = (module.getOrInsertFunction(
            "air." + (trail ? std::string("ctz") : std::string("clz")) +
                type_overload_suffix(operand_type, Sign::inapplicable),
            llvm::FunctionType::get(operand_type,
                                    {operand_type, ctx.types._bool}, false),
            att));
        ...
        if (isa<VectorType>(operand_type)) {
            ...
            return ctx.builder.CreateSelect(
                ctx.builder.CreateICmpEQ(
                    ret, ctx.builder.CreateVectorSplat(
                             comp_num,
                             ctx.builder.getIntN(comp_type->getBitWidth(),
                                                 comp_type->getBitWidth()))),
            ...
            return ctx.builder.CreateSelect(
                ctx.builder.CreateICmpEQ(
                    ret, ctx.builder.getIntN(int_type->getBitWidth(),
...
auto get_operation_info(MSLTexture texture)
    ...
    auto& types = ctx.types;
    auto sign = std::visit(patterns{[](MSLUint) { return Sign::no_sign; },
    ...
    case TextureKind::depth_2d: {
        ...
        ret.gradient_type = types._float2;
        ret.gather_type =
            types._float4; // gather returns float4 instead of float
        ret.address_type = types._int2;
        break;
    }
...
AIRBuilderResult call_sample(MSLTexture texture_type, pvalue handle,
                             pvalue sampler, pvalue coord, pvalue array_index,
                             pvalue offset, pvalue bias, pvalue min_lod_clamp,
                             pvalue lod_level) {
    auto ctx = co_yield get_context();
    using namespace llvm;
    auto& context = ctx.llvm;
    ...
    if (op_info.offset_type != nullptr) {
        ...
        args_value.push_back(offset != nullptr ? offset
    ...
    if (lod_level != nullptr) {
        ...
        args_value.push_back(min_lod_clamp != nullptr
                                 ? min_lod_clamp
...
AIRBuilderResult call_sample_grad(MSLTexture texture_type, pvalue handle,
                                  pvalue sampler_handle, pvalue coord,
                                  pvalue array_index, pvalue dpdx, pvalue dpdy,
                                  pvalue minlod, pvalue offset) {
    auto ctx = co_yield get_context();
    using namespace llvm;
    auto& context = ctx.llvm;
    auto& module = ctx.module;
    ...
    if (op_info.offset_type != nullptr) {
        args_type.push_back(types._bool);
        args_value.push_back(
            ctx.builder.getInt1(op_info.sample_unknown_bit)); // = 1
        args_type.push_back(op_info.offset_type);
        args_value.push_back(offset != nullptr ? offset
                                               : ConstantAggregateZero::get(
                                                     op_info.offset_type));
...
AIRBuilderResult call_sample_compare(MSLTexture texture_type, pvalue handle,
                                     pvalue sampler_handle, pvalue coord,
                                     pvalue array_index, pvalue reference,
                                     pvalue offset, pvalue bias,
    ...
    /* reference */
    args_type.push_back(types._float);
    args_value.push_back(reference ? reference
                                   : ConstantFP::get(types._float, 0));
...
AIRBuilderResult call_gather(MSLTexture texture_type, pvalue handle,
                             pvalue sampler_handle, pvalue coord,
                             pvalue array_index, pvalue offset,
                             pvalue component) {
    auto ctx = co_yield get_context();
    using namespace llvm;
    auto& context = ctx.llvm;
...
AIRBuilderResult call_gather_compare(MSLTexture texture_type, pvalue handle,
                                     pvalue sampler_handle, pvalue coord,
                                     pvalue array_index, pvalue reference,
                                     pvalue offset) {
    auto ctx = co_yield get_context();
...
AIRBuilderResult call_read(MSLTexture texture_type, pvalue handle,
                           pvalue address, pvalue offset, pvalue cube_face,
                           pvalue array_index, pvalue sample_index,
                           pvalue lod) {
    auto ctx = co_yield get_context();
    using namespace llvm;
    auto& context = ctx.llvm;
    ...
    args_type.push_back(types._int);
    args_value.push_back(
        ctx.builder.getInt32((uint32_t)texture_type.memory_access));
...
AIRBuilderResult call_write(MSLTexture texture_type, pvalue handle,
                            pvalue address, pvalue cube_face,
...
AIRBuilderResult call_calc_lod(MSLTexture texture_type, pvalue handle,
                               pvalue sampler, pvalue coord,
                               bool is_unclamped) {
    auto ctx = co_yield get_context();
    using namespace llvm;
    auto& context = ctx.llvm;
...
AIRBuilderResult call_get_texture_info(MSLTexture texture_type, pvalue handle,
                                       TextureInfoType type, pvalue lod) {
    auto ctx = co_yield get_context();
    using namespace llvm;
    auto& context = ctx.llvm;
    auto& module = ctx.module;
...
AIRBuilderResult call_texture_atomic_fetch_explicit(
    MSLTexture texture_type, pvalue handle, std::string op, bool is_signed,
    ...
    auto fn_name =
        ...
        type_overload_suffix(vec4->getType(),
                             is_signed ? Sign::with_sign : Sign::no_sign);
    std::vector<llvm::Type*> args_type;
    std::vector<pvalue> args_value;
    ...
    auto fn = module.getOrInsertFunction(
        fn_name, llvm::FunctionType::get(op_info.write_type, args_type, false),
        att);
    co_return ctx.builder.CreateExtractElement(
        ctx.builder.CreateCall(fn, args_value), uint64_t(0));
};

AIRBuilderResult call_texture_atomic_exchange(MSLTexture texture_type,
                                              pvalue handle, pvalue address,
    ...
    auto fn_name = "air.atomic_exchange_explicit_" + op_info.air_symbol_suffix +
                   type_overload_suffix(vec4->getType(), Sign::no_sign);
    std::vector<llvm::Type*> args_type;
    std::vector<pvalue> args_value;
...
AIRBuilderResult
call_texture_atomic_compare_exchange(MSLTexture texture_type, pvalue handle,
                                     pvalue address, pvalue array_index,
                                     pvalue compared, pvalue operand) {
    auto ctx = co_yield get_context();
    using namespace llvm;
    auto& context = ctx.llvm;
    auto& module = ctx.module;
    ...
    assert(!op_info.is_ms);
    assert(op_info.write_type == types._int4);
    auto fn_name = "air.atomic_compare_exchange_weak_explicit_" +
                   op_info.air_symbol_suffix +
                   type_overload_suffix(operand->getType(), Sign::no_sign);
    std::vector<llvm::Type*> args_type;
    std::vector<pvalue> args_value;
    ...
    auto fn = module.getOrInsertFunction(
        fn_name, llvm::FunctionType::get(ctx.types._bool, args_type, false),
        att);
...
AIRBuilderResult call_convert(pvalue src, llvm::Type* dst_scaler_type,
                              Sign sign) {
    using namespace llvm;
    auto ctx = co_yield get_context();
    auto& context = ctx.llvm;
    ...
    if (isa<llvm::FixedVectorType>(src_type)) {
        dst_type = llvm::FixedVectorType::get(
            dst_scaler_type,
            cast<llvm::FixedVectorType>(src_type)->getNumElements());
    } else {
...
AIRBuilderResult call_atomic_fetch_explicit(pvalue pointer, pvalue operand,
                                            std::string op, bool is_signed,
                                            bool device) {
    using namespace llvm;
    auto ctx = co_yield get_context();
    auto& context = ctx.llvm;
    ...
    auto fn = (module.getOrInsertFunction(
        ...
        llvm::FunctionType::get(
            ...
            false),
        att));
    co_return ctx.builder.CreateCall(fn, {pointer, operand, co_yield get_int(0),
                                          co_yield get_int(device ? 2 : 1),
                                          ConstantInt::getBool(context, true)});
};

AIRBuilderResult call_atomic_exchange_explicit(pvalue pointer, pvalue operand,
                                               bool device) {
    using namespace llvm;
    auto ctx = co_yield get_context();
    auto& context = ctx.llvm;
...
AIRBuilderResult call_atomic_cmp_exchange(pvalue pointer, pvalue compared,
                                          pvalue operand, pvalue tmp_mem,
                                          bool device) {
    using namespace llvm;
    auto ctx = co_yield get_context();
    auto& context = ctx.llvm;
    ...
    ctx.builder.CreateStore(compared, ptr);
    ctx.builder.CreateCall(fn, {pointer, alloca, operand, co_yield get_int(0),
                                co_yield get_int(0),
                                co_yield get_int(device ? 2 : 1),
                                ConstantInt::getBool(context, true)});
...
AIRBuilderResult call_unpack_impl(std::string op, pvalue src,
                                  llvm::Type* src_type, llvm::Type* dst_type) {
    using namespace llvm;
    auto ctx = co_yield get_context();
    auto& context = ctx.llvm;
    auto& module = ctx.module;
...
AIRBuilderResult load_from_device_buffer(llvm::Type* value_type,
                                         pvalue base_addr,
                                         pvalue aligned_offest,
    ...
    co_return ctx.builder.CreateAlignedLoad(
        ...
        ctx.builder.CreateConstGEP1_32(
            ...
            ctx.builder.CreateBitCast(
                ctx.builder.CreateGEP(ctx.types._byte, base_addr,
                                      {aligned_offest}),
...
AIRBuilderResult unpack_fvec4_from_addr(MTLAttributeFormat format,
                                        pvalue base_addr,
                                        pvalue aligned_offest) {
    auto ctx = co_yield get_context();
    auto& types = ctx.types;
    ...
    pvalue ret = co_yield call_unpack_impl(
        op,
        co_yield load_from_device_buffer(src_type, base_addr, aligned_offest, 0,
                                         align),
        src_type, dst_type);
    ...
    if (dst_type == types._float) {
        ret = builder.CreateInsertElement(llvm::PoisonValue::get(types._float4),
                                          ret, (int)0);
...
AIRBuilderResult pull_vec4_from_addr_checked(MTLAttributeFormat format,
                                             pvalue base_addr,
                                             pvalue byte_offset) {
    auto ctx = co_yield get_context();
    auto& types = ctx.types;
    ...
    case MTLAttributeFormat::Char:
        value = co_yield load_from_device_buffer(types._byte, base_addr,
                                                 byte_offset, 0, 1);
        value = builder.CreateSExt(value, types._int);
        value = builder.CreateInsertElement(llvm::PoisonValue::get(types._int4),
                                            value, (int)0);
    ...
    case MTLAttributeFormat::Char2:
        value = co_yield load_from_device_buffer(types._char2, base_addr,
                                                 byte_offset, 0, 1);
    ...
    case MTLAttributeFormat::Char4:
        value = co_yield load_from_device_buffer(types._char4, base_addr,
                                                 byte_offset, 0, 1);
    ...
    case MTLAttributeFormat::Short:
        value = co_yield load_from_device_buffer(types._short, base_addr,
                                                 byte_offset, 0, 2);
        value = builder.CreateSExt(value, types._int);
        value = builder.CreateInsertElement(llvm::PoisonValue::get(types._int4),
                                            value, (int)0);
    ...
    case MTLAttributeFormat::Short2:
        value = co_yield load_from_device_buffer(types._short2, base_addr,
                                                 byte_offset, 0, 2);
        value = builder.CreateSExt(value, types._int2);
        value = builder.CreateShuffleVector(value, {0, 1, -1, -1});
        value = builder.CreateInsertElement(value, co_yield get_int(0), (int)2);
    ...
    case MTLAttributeFormat::Short4:
        value = co_yield load_from_device_buffer(types._short4, base_addr,
                                                 byte_offset, 0, 2);
        value = builder.CreateSExt(value, types._int4);
        break;
    ...
    case MTLAttributeFormat::Half:
        value = co_yield load_from_device_buffer(types._half, base_addr,
                                                 byte_offset, 0, 2);
        ...
        value =
            builder.CreateInsertElement(value, co_yield get_float(0), (int)1);
        value =
            builder.CreateInsertElement(value, co_yield get_float(0), (int)2);
        value =
            builder.CreateInsertElement(value, co_yield get_float(1), (int)3);
        break;
    case MTLAttributeFormat::Half2:
        value = co_yield load_from_device_buffer(types._half2, base_addr,
                                                 byte_offset, 0, 2);
        value = co_yield call_convert(value, types._float, Sign::with_sign);
        value = builder.CreateShuffleVector(value, {0, 1, -1, -1});
        value =
    ...
    case MTLAttributeFormat::Half4:
        value = co_yield load_from_device_buffer(types._half4, base_addr,
                                                 byte_offset, 0, 2);
        value = co_yield call_convert(value, types._float, Sign::with_sign);
        break;
    case MTLAttributeFormat::Float:
        value = co_yield load_from_device_buffer(types._float, base_addr,
                                                 byte_offset, 0, 4);
        value = builder.CreateInsertElement(
            llvm::PoisonValue::get(types._float4), value, (int)0);
        value =
    ...
    case MTLAttributeFormat::Float2:
        value = co_yield load_from_device_buffer(types._float2, base_addr,
                                                 byte_offset, 0, 4);
        value = builder.CreateShuffleVector(value, {0, 1, -1, -1});
        value =
            builder.CreateInsertElement(value, co_yield get_float(0), (int)2);
    ...
    case MTLAttributeFormat::Float3:
        value = co_yield load_from_device_buffer(types._float3, base_addr,
                                                 byte_offset, 0, 4);
    ...
    case MTLAttributeFormat::Float4:
        value = co_yield load_from_device_buffer(types._float4, base_addr,
                                                 byte_offset, 0, 4);
        break;
    case MTLAttributeFormat::UInt:
    case MTLAttributeFormat::Int:
        value = co_yield load_from_device_buffer(types._int, base_addr,
                                                 byte_offset, 0, 4);
        value = builder.CreateInsertElement(llvm::PoisonValue::get(types._int4),
                                            value, (int)0);
        value = builder.CreateInsertElement(value, co_yield get_int(0), (int)1);
    ...
    case MTLAttributeFormat::Int2:
        value = co_yield load_from_device_buffer(types._int2, base_addr,
                                                 byte_offset, 0, 4);
        value = builder.CreateShuffleVector(value, {0, 1, -1, -1});
        value = builder.CreateInsertElement(value, co_yield get_int(0), (int)2);
        value = builder.CreateInsertElement(value, co_yield get_int(1), (int)3);
    ...
    case MTLAttributeFormat::Int3:
        value = co_yield load_from_device_buffer(types._int3, base_addr,
                                                 byte_offset, 0, 4);
        value = builder.CreateShuffleVector(value, {0, 1, 2, -1});
        value = builder.CreateInsertElement(value, co_yield get_int(1), (int)3);
        break;
    ...
    case MTLAttributeFormat::Int4:
        value = co_yield load_from_device_buffer(types._int4, base_addr,
                                                 byte_offset, 0, 4);
        break;
    case MTLAttributeFormat::UCharNormalized:
    case MTLAttributeFormat::UChar2Normalized:
...
AIRBuilderResult pull_vec4_from_addr(MTLAttributeFormat format,
                                     pvalue base_addr, pvalue byte_offset) {
    return make_op([=](struct AIRBuilderContext ctx) -> llvm::Expected<pvalue> {
        ...
        auto is_null_binding = builder.CreateICmpEQ(
            builder.CreatePtrToInt(base_addr, ctx.types._long),
            builder.getInt64(0));
        ...
        auto ret = pull_vec4_from_addr_checked(format, base_addr, byte_offset)
                       .build(ctx);
...
AIRBuilderResult call_set_mesh_properties(pvalue mesh_grid_props,
                                          pvalue grid_size) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
        ...
        auto fn = (module.getOrInsertFunction(
            ...
            llvm::FunctionType::get(
                ...
                {ctx.types._mesh_grid_properties->getPointerTo(3),
                 ctx.types._int3},
                false),
            att));
...
AIRBuilderResult call_interpolate_at_center(pvalue interpolant,
                                            bool perspective) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
        ...
        auto fn = (module.getOrInsertFunction(
            ...
            llvm::FunctionType::get(ctx.types._float4,
                                    {ctx.types._interpolant->getPointerTo(1)},
                                    false),
            att));
...
AIRBuilderResult call_interpolate_at_centroid(pvalue interpolant,
                                              bool perspective) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
...
AIRBuilderResult call_interpolate_at_offset(pvalue interpolant,
                                            bool perspective, pvalue offset) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
        auto& module = ctx.module;
        ...
        auto fn = (module.getOrInsertFunction(
            ...
            llvm::FunctionType::get(
                ctx.types._float4,
                {ctx.types._interpolant->getPointerTo(1), ctx.types._float2},
                false),
            att));
...
AIRBuilderResult call_interpolate_at_sample(pvalue interpolant,
                                            bool perspective, pvalue index) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
        auto& module = ctx.module;
        ...
        auto fn = (module.getOrInsertFunction(
            ...
            llvm::FunctionType::get(
                ctx.types._float4,
                {ctx.types._interpolant->getPointerTo(1), ctx.types._int},
                false),
            att));
...
AIRBuilderResult
call_set_mesh_render_target_array_index(pvalue mesh, pvalue vid,
                                        pvalue render_target_array_index) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
        auto& module = ctx.module;
        ...
        auto fn = (module.getOrInsertFunction(
            ...
            llvm::FunctionType::get(
                llvm::Type::getVoidTy(context),
                {ctx.types._mesh->getPointerTo(7 /* mesh */), ctx.types._int,
                 ctx.types._int},
                false),
            att));
        return ctx.builder.CreateCall(fn,
                                      {mesh, vid, render_target_array_index});
    });
}

AIRBuilderResult
call_set_mesh_viewport_array_index(pvalue mesh, pvalue vid,
                                   pvalue viewport_array_index) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
        auto& module = ctx.module;
...
AIRBuilderResult call_set_mesh_position(pvalue mesh, pvalue vid,
                                        pvalue position) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
...
AIRBuilderResult call_set_mesh_clip_distance(pvalue mesh, uint32_t idx,
                                             pvalue vid, pvalue value) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
        auto& module = ctx.module;
        ...
        auto fn = (module.getOrInsertFunction(
            ...
            llvm::FunctionType::get(
                llvm::Type::getVoidTy(context),
                {ctx.types._mesh->getPointerTo(7 /* mesh */), ctx.types._int,
                 ctx.types._int, ctx.types._float},
                false),
            att));
        return ctx.builder.CreateCall(
            fn, {mesh, ctx.builder.getInt32(idx), vid, value});
    });
}

AIRBuilderResult call_set_mesh_vertex_data(pvalue mesh, uint32_t idx,
                                           pvalue vid, pvalue value) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
...
AIRBuilderResult call_set_mesh_primitive_data(pvalue mesh, uint32_t idx,
                                              pvalue pid, pvalue value) {
    return make_op([=](struct AIRBuilderContext ctx) {
        using namespace llvm;
        auto& context = ctx.llvm;
        auto& module = ctx.module;
        ...
        auto fn = (module.getOrInsertFunction(
            ...
            att));
        return ctx.builder.CreateCall(
            fn, {mesh, ctx.builder.getInt32(idx), pid, value});
    });
}
...
AIRBuilderResult call_set_mesh_index(pvalue mesh, pvalue index, pvalue vertex) {
    return make_op([=](struct AIRBuilderContext ctx) {
        ...
        return ctx.builder.CreateCall(
            fn, {mesh, index,
                 ctx.builder.CreateZExtOrTrunc(vertex, ctx.types._byte)});
    });
}
