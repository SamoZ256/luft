        ...
        if (vertexFns) {
            for (auto fn : vertexFns->operands()) {
                ...
                auto func = dyn_cast<Function>(
                    dyn_cast<ConstantAsMetadata>(fn->getOperand(0).get())
                        ->getValue());
                ...
                while (fn->getNumOperands() > 3 &&
                       isa<MDTuple>(fn->getOperand(3).get())) {
                    auto maybe_patch_tuple =
                        cast<MDTuple>(fn->getOperand(3).get());
                    ...
                    if (air_patch_value->getString() == "triangle") {
                        function_def_stream << value(MTLB_TESS_TAG{
                            .patchType = 1, .controlPointCount = 0});
                    } else {
                        function_def_stream << value(MTLB_TESS_TAG{
                            .patchType = 2, .controlPointCount = 0});
                    }
                    break;
                }
                ...
                for (auto& input : inputs->operands()) {
                    ...
                    auto inputKind =
                        dyn_cast<MDString>(inputElement->getOperand(1))
                            ->getString();
                    if (inputKind == "air.vertex_input") {
                        uint32_t location = dyn_cast<ConstantInt>(
                                                dyn_cast<ConstantAsMetadata>(
                                                    inputElement->getOperand(3))
                                                    ->getValue())
                                                ->getValue()
                                                .getZExtValue();
                        auto typeName =
                            dyn_cast<MDString>(inputElement->getOperand(6))
                                ->getString();
                        auto argName =
                            dyn_cast<MDString>(inputElement->getOperand(8))
                                ->getString();
                ...
                raw_svector_ostream fn_public_metadata_stream(
                    fn_public_metadata);
                if (attribtues.size()) {
                    // if no vertex attribtues, then don't emit VATY, otherwise
                    // PSO doesn't compile
                    fn_public_metadata_stream
                        << value(MTLBFourCC::VertexAttribute);
                    auto lenOffset = fn_public_metadata_stream.tell();
                    fn_public_metadata_stream << value((uint16_t)0);
                    fn_public_metadata_stream
                        << value((uint16_t)attribtues.size());
                    ...
                    auto vatt_written =
                        fn_public_metadata_stream.tell() - lenOffset;
                    *(uint16_t*)(&fn_public_metadata[lenOffset]) =
                        vatt_written - 2;
                    fn_public_metadata_stream
                        << value(MTLBFourCC::VertexAttributeType);
                    fn_public_metadata_stream
                        << value((uint16_t)(2 + attribtues.size()));
                    fn_public_metadata_stream
                        << value((uint16_t)(attribtues.size()));
                    for (auto& vattr : attribtues) {
                ...
                public_metadata_stream
                    << value((uint32_t)fn_public_metadata.size());
        ...
        if (kernelFns) {
            for (auto fn : kernelFns->operands()) {
                ...
                raw_svector_ostream fn_public_metadata_stream(
                    fn_public_metadata);
                // fn_public_metadata_stream <<
                // value(MTLBFourCC::VertexAttribute); auto lenOffset =
                // fn_public_metadata_stream.tell(); fn_public_metadata_stream
                // << value((uint16_t)0); fn_public_metadata_stream <<
                // value((uint16_t)attribtues.size()); for (auto &vattr :
                // attribtues) {
                //   fn_public_metadata_stream << vattr.name << '\0';
                //   fn_public_metadata_stream << value(MTLB_VATY{
                //     .attribute = vattr.attribute,
                ...
                //   });
                // }
                // auto vatt_written = fn_public_metadata_stream.tell() -
                // lenOffset;
                // *(uint16_t *)(&fn_public_metadata[lenOffset]) = vatt_written
                // - 2; fn_public_metadata_stream <<
                // value(MTLBFourCC::VertexAttributeType);
