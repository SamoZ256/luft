...
inline llvm::StructType* getOrCreateStructType(llvm::StringRef Name,
                                               llvm::LLVMContext& Ctx) {
    using namespace llvm;
    StructType* ST = StructType::getTypeByName(Ctx, Name);
    if (ST)
...
constexpr auto get_llvm_type = [](auto msl_type, llvm::LLVMContext& context) {
    return std::visit([&](auto x) { return x.get_llvm_type(context); },
                      msl_type);
};
...
using MSLScalerType =
    std::variant<MSLFloat, MSLInt, MSLUint, MSLBool, MSLHalf, MSLUshort,
...
struct MSLVector {
    ...
    llvm::Type* get_llvm_type(llvm::LLVMContext& context) const {
        auto scaler_type = std::visit(
            [&](auto x) { return x.get_llvm_type(context); }, scaler);
        if (dimension == 1) {
            return scaler_type;
        }
...
struct MSLTexture {
    ...
    std::string get_name() const {
        auto component = std::visit(
            [](auto scaler) { return scaler.get_name(); }, component_type);
    ...
    llvm::Type* get_llvm_type(llvm::LLVMContext& context) const {
        ...
        case TextureKind::texture_2d_ms:
            return getOrCreateStructType("struct._texture_2d_ms_t", context)
                ->getPointerTo(1);
        case TextureKind::texture_2d_ms_array:
            return getOrCreateStructType("struct._texture_2d_ms_array_t",
                                         context)
                ->getPointerTo(1);
        case TextureKind::texture_3d:
        ...
        case TextureKind::texture_cube:
            return getOrCreateStructType("struct._texture_cube_t", context)
                ->getPointerTo(1);
        case TextureKind::texture_cube_array:
            return getOrCreateStructType("struct._texture_cube_array_t",
                                         context)
                ->getPointerTo(1);
        case TextureKind::texture_buffer:
...
struct MSLPointer {
    ...
    llvm::Type* get_llvm_type(llvm::LLVMContext& context) {
        auto pointee_type = std::visit(
            [&](auto x) { return x.get_llvm_type(context); }, pointee);
...
struct MSLStaticArray {
    ...
    llvm::Type* get_llvm_type(llvm::LLVMContext& context) {
        return llvm::ArrayType::get(
            std::visit([&](auto x) { return x.get_llvm_type(context); },
                       element_type),
            array_size);
    }
};
...
class ArgumentBufferBuilder {
    /* the return value is the element index of structure (as well as in
     * argument buffer) */
  public:
    uint32_t
    DefineBuffer(std::string name, AddressSpace addressp_space,
    ...
    // );
    uint32_t
    DefineTexture(std::string name, TextureKind kind, MemoryAccess access,
                  MSLScalerType scaler_type,
                  uint32_t location_index = UINT32_MAX,
                  std::optional<uint32_t> raster_order_group = std::nullopt);
    uint32_t DefineSampler(std::string name,
                           uint32_t location_index = UINT32_MAX);
    uint32_t DefineInteger64(std::string name,
                             uint32_t location_index = UINT32_MAX);
...
struct OutputMeshData {
    ...
    bool operator==(OutputMeshData const& rhs) const {
...
struct OutputRenderTarget {
    ...
    bool operator==(OutputRenderTarget const& rhs) const {
...
struct OutputRenderTargetArrayIndex {
    bool operator==(OutputRenderTargetArrayIndex const& rhs) const {
        return true;
    }
};
...
using MeshVertexOutput =
    std::variant<OutputMeshData, OutputPosition, OutputClipDistance>;

using MeshPrimitiveOutput =
    std::variant<OutputMeshData, OutputRenderTargetArrayIndex,
                 OutputViewportArrayIndex>;
  ...
  public:
    ...
    PostTessellationPatch GetPatchType() {
        assert(patch.has_value() &&
               "not a domain shader/unsupported isoline tessellation");
        return patch->first;
    };

    auto CreateFunction(std::string name, llvm::LLVMContext& context,
                        llvm::Module& module, uint64_t sign_mask,
                        bool skip_output)
...
inline TextureKind to_air_resource_type(shader::common::ResourceType type,
    ...
    case shader::common::ResourceType::Texture2DMultisampled:
        return use_depth ? TextureKind::depth_2d_ms
                         : TextureKind::texture_2d_ms;
...
inline MSLScalerType to_air_scaler_type(shader::common::ScalerDataType type) {
    switch (type) {
